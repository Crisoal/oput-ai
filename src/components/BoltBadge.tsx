...
export const BoltBadge: React.FC<BoltBadgeProps> = ({ 
  ...
  // For white variant, use the provided image
  if (variant === 'white') {
    return (
      <button
        onClick={handleClick}
  ...
  return (
    <button
      onClick={handleClick}
      className={`flex items-center justify-center w-12 h-12 rounded-full transition-all duration-200 hover:scale-105 bg-black text-white hover:bg-gray-900 ${className}`}
      title="Built with Bolt.new"
    >
      <svg
        width="20"
